...
??? question "How to populate `--puck-barcode-file`"
    With Open-ST data, each sample covers a piece of capture area, which contains at least one tile (puck).

    Thus, we need to provide `--puck-barcode-file` (each tile in a sample has different barcodes, unlike for visium samples).
    ...
    need to be specified after `--puck-barcode-file`, e.g., with the wildcards `/path/to/fc_tiles/*.txt.gz`.
