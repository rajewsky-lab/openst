...
def prepare_image_for_feature_matching(
    ...
    Args:
        ...
        keep_black_background (bool, optional): If True, keeps background pixels as black. Only used with mask_tissue=True
    ...
    def _transform(im, flip, crop):
        return gaussian(equalize_adapthist(im)[:: flip[0], :: flip[1]].copy(), gaussian_blur)[
...
def run_registration(
    ...
    for _flip_x, _flip_y in _flips:
        print(f"### Flip {_flip_x, _flip_y}")
        ...
        _, inliers = ransac(
            (
                mkpts0_all_flip[f"[::{_flip_x},::{_flip_y}]"],
                mkpts1_all_flip[f"[::{_flip_x},::{_flip_y}]"],
            ),
            SimilarityTransform,
    ...
    print("### Final optimal flip")
    # _best_inlier = np.argmax([inliers.sum() for inliers in list(inliers_all_flip.values())])
    _best_inlier = np.argmax([len(inliers) for inliers in list(inliers_all_flip.values())])
    _key_best_inlier = list(inliers_all_flip.keys())[_best_inlier]
    inliers = inliers_all_flip[_key_best_inlier]
    _best_flip = _flips[_best_inlier]

    # Filter keypoints with selected inliers
    in_mkpts0 = mkpts0_all_flip[_key_best_inlier][inliers.flatten()]
    in_mkpts1 = mkpts1_all_flip[_key_best_inlier][inliers.flatten()]
    print(
        f"""- Kept *{_best_flip} flip* with *{inliers.sum()} inliers*
        out of *{len(mkpts0_all_flip[_key_best_inlier])} matches*"""
    )
    ...
    if args.only_coarse:
        return out_coords_output_coarse, None, staining_image[:: _best_flip[0], :: _best_flip[1]], metadata
    ...
    for tile_code in tile_codes:
        ...
        _t_mkpts0, _t_mkpts1 = find_matches(src, dst)
        ...
        if (
            ...
            or (_t_tform_points.translation.min() > _t_sts_coords_fine_to_transform.max(axis=0))
        ):
    ...
    return (
        ...
        staining_image[:: _best_flip[0], :: _best_flip[1]],
        metadata,
    )
